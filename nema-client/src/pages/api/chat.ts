...
const handleRequest = async ({ prompt, userId }: { prompt: string; userId: string }) => {
  ...
  try {
    ...
    /// 2) Step 2 - New - Combine this info with a new prompt and call OpenAPI to produce a set of results.  Here we have to make sure the source referes to the  link returned by Step 1, as we wan to show it to the user
