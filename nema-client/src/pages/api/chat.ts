...
const handleRequest = async ({ prompt, userId }: { prompt: string; userId: string }) => {
  ...
  try {
    ...
    // const response = await babyAGI.call({ objective: 'What is impermanent loss?' });
    // console.log('babyAGI:', response);
    ...
    // const response = await autogpt.run(['write a weather report for SF today']);
    // console.log('autogpt: ', response);
  } catch (error) {
    //@ts-ignore
    console.error(error);
  }
