...
export async function getTestNewsDocs(): Promise<LGCDocument<PageMetadata>[]> {
  return testNews2
    ...
    .map((news) => {
      const processedSummary: string = preprocess(news.summary);
      return new LGCDocument<PageMetadata>({
        pageContent: processedSummary,
...
function preprocess(sentence?: string): string {
  if (sentence) {
    const punctRegex = /[^\w\s]|_/g;
    const spaceRegex = /\s+/g;

    const cleaned: string[] = sentence.replace(punctRegex, '').replace(spaceRegex, ' ').split(' ').map(lemmatizer);

    //convert array into sentence and log it
    const cleanedSentence = cleaned.join(' ');
