...
const testNews2: { summary: string; url: string }[] = [
  ...
  {
    summary: 'In a bid to allay market flooding concerns and provide certainty to XRP holders, Ripple commits to locking up $14 billion in XRP cryptocurrency.',
    url: 'https://www.coindesk.com/markets/2017/05/16/ripple-pledges-to-lock-up-14-billion-in-xrp-cryptocurrency/',
  },
  {
    summary:
      'To alleviate market concerns and instill confidence among XRP owners and prospective investors, Ripple announces plans to secure $14 billion worth of XRP by locking it away.',
    url: 'https://www.coindesk.com/learn/what-is-ripple-and-the-xrp-cryptocurrency/',
  },
  {
    summary:
      'Proposed US crypto bill aims to hold SEC accountable for regulatory overreach. Amendment suggests SEC should reimburse defendants legal fees if they win cases. Ripple CEO claims $200 million spent on defense in SEC lawsuit.',
    url: 'https://www.coindesk.com/markets/2020/12/04/ripple-cto-says-majority-vote-would-force-the-burning-of-billions-in-xrp/',
  },
];
...
export async function getAllExistingNewsSubSetVectors(pineconeIndex: VectorOperationsApi): Promise<Vector[]> {
  const result = await pineconeIndex.fetch({
    namespace: LATEST_NEWS_NAMESPACE,
    ids: [
...
export async function getAllExistingCuratedNewsVectors(pineconeIndex: VectorOperationsApi): Promise<Vector[]> {
  const result = await pineconeIndex.fetch({
    namespace: LATEST_NEWS_NAMESPACE,
    ids: [
