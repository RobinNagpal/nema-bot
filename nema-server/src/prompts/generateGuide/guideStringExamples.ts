export const impermanentLossGuideString = `
Impermanent loss is a temporary loss of funds occurring when providing liquidity. It is caused by the volatility of the pool's assets. 
The more volatile the assets, the higher the impermanent loss.


https://medium.com/auditless/impermanent-loss-in-uniswap-v3-6c7161d3b445

https://mymerlin.io/website/track-impermanent-loss-on-uniswap-v3/

https://thedefiant.io/uniswap-v3-impermanent-loss

https://docs.uniswap.org/contracts/v2/concepts/advanced-topics/understanding-returns

https://www.flovtec.com/post/market-making-solution-to-impermanent-loss-problem-on-uniswap-v3

https://www.shipyardsoftware.org/post/impermanent-loss-is-a-choice

https://bdc.consulting/insights/options/integrating-options-are-an-effective-hedge
`;

export const impermanentLossGuideDirections = `
  Explain impermanent loss
 
  Give a few concrete examples of impermanent loss
   
  Explain how to calculate impermanent loss

  Explain how to minimize impermanent loss

  Explain how to track impermanent loss

  Explain how to hedge impermanent loss
`;

export const concentratedLiquidityGuideString = `
  Concentrated liquidity is a new way of providing liquidity that allows you to choose the price range in which you want to provide liquidity.
  This allows you to provide liquidity in a way that is more capital efficient and less exposed to impermanent loss.

  https://docs.uniswap.org/concepts/protocol/concentrated-liquidity
  
  https://itsa-global.medium.com/defi-insight-concentrated-liquidity-on-uniswap-v3-9dce4e67c3e9
  
  https://uniswapv3book.com/docs/introduction/uniswap-v3/
  
  https://coinmarketcap.com/alexandria/glossary/concentrated-liquidity
  
  https://levelup.gitconnected.com/whats-new-in-uniswap-v3-a04a8f2673bb
  
  
  https://blog.hubbleprotocol.io/what-is-clmm/
  
  https://www.coingecko.com/learn/uniswap-vs-trader-joe
  
  https://pontem.network/posts/concentrated-liquidity-beginner-guide-to-uniswap-v3-liquidity-book
`;

export const concentratedLiquidityGuideDirections = `
`;
